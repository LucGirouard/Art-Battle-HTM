  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      if (artworks) {
        ...
        setTodaySubmitted(artworks.some((a) => a.user_id === uid));
      }
    });
  }, []);
    ...
    <PageShell maxWidth="4xl">
      <PageCard className="animate-[rise-in_700ms_ease-out] px-5 py-9 sm:px-8 sm:py-11 md:px-12">
        ...
        <PageTitle className="mt-4 text-4xl sm:text-5xl">
          Rate today&apos;s daily draw
        </PageTitle>
        <p className="mt-4 max-w-2xl text-base leading-7 text-stone-700">
          Swipe to vote and help rank today&apos;s leaderboard.
        </p>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border border-white/70 bg-white/70 p-4">
            ...
            <p className="mt-2 text-base font-semibold text-stone-900">
              {todaySubmitted ? "Submitted today" : "No daily draw submitted yet"}
            </p>
          </div>
          <div className="rounded-2xl border border-white/70 bg-white/70 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-stone-500">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          <PrimaryButton
            onClick={() => router.push(ROUTES.rateItArena)}
